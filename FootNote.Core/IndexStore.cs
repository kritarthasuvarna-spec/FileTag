...
/// <summary>
/// Local lookup cache + app config, stored at %LocalAppData%\FootNote\index.json.
/// Tracks which paths currently carry a FootNote comment (paths only, never text).
...
public sealed class IndexStore
{
    ...
    private static IndexData Load()
    {
        try
        {
            ...
            {
                var data = JsonSerializer.Deserialize(File.ReadAllText(IndexFilePath), FootNoteJsonContext.Default.IndexData);
                if (data is not null)
                {
    ...
    private void Save()
    {
        try
        {
            ...
            File.WriteAllText(IndexFilePath, JsonSerializer.Serialize(_data, FootNoteJsonContext.Default.IndexData));
        }
