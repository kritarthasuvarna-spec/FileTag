...
/// %APPDATA%\FootNote\logs\, trimmed to ~7 days. Setup writes install.log in
...
public static class Logger
{
    ...
    public static string LogsDirectory => Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "FootNote", "logs");

    public static string AppLogPath => Path.Combine(LogsDirectory, "footnote.log");
    public static string InstallLogPath => Path.Combine(LogsDirectory, "install.log");
    public static string UninstallLogPath => Path.Combine(Path.GetTempPath(), "FootNote-uninstall.log");
        ...
        try
        {
            ...
            if (fi.Exists && (fi.LastWriteTime.Date < DateTime.Now.Date || fi.Length > MaxSizeBytes))
            {
                string archived = Path.Combine(LogsDirectory,
                    $"footnote-{fi.LastWriteTime:yyyyMMdd-HHmmss}.log");
                if (!File.Exists(archived)) File.Move(AppLogPath, archived);
            }

            foreach (var old in new DirectoryInfo(LogsDirectory).GetFiles("footnote-*.log"))
            {
